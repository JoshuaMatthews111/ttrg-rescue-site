...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const name = data?.name || "Dog Profile";
  const description = data?.story || data?.rescue_story || `Meet ${name} at Team Trainers Rescue Group. Support their journey today.`;
  const image = data?.image || "/ttrg/ttrg-logo.png";
